...
class TrainingProcess:
    ...
    def validate(self, data_path=None):
        ...
        if data_path:
            self.lines_train, self.lines_val = self._data.read_training_data(data_path)
        print("inside client1_new.validate")

        self.local_model.compile(optimizer=Adam(lr=self.lr),
                                 loss={'yolo_loss': lambda y_true, y_pred: y_pred})  # recompile to apply the change
        print("client1_new.validate compiles")
        # batch_size = 32 # note that more GPU memory is required after unfreezing the body
        self.logger.info('Train on {} samples, val on {} samples, with batch size {}.'
                         .format(len(self.lines_train), len(self.lines_val), self._data.batch_size))
        print("client1_new.validate before train evaluate")
        ...
        print("client1_new.validate after train evaluate before val evaluate")
        ...
        val_results = self.local_model.evaluate(
            ...
            steps=max(1, len(self.lines_train) // self._data.batch_size))
        print("client1_new.validate after val evaluate")
